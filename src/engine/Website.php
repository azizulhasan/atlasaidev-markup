...
class Website extends BaseEngine {
    ...
    protected function url() {
        ...
        if (!empty($url)) {
            return apply_filters("markup_{this->schema_type}_url", $url);
        }
    ...
    protected function headline() {
        ...
        if (!empty($headline)) {
            return apply_filters("markup_{this->schema_type}_headline", $headline);
        }
...
//        error_log( print_r( wc_get_product( $this->post_id->get_image_url() ), true ) );
        ...
        if ( ! empty( $image ) ) {
            return apply_filters("markup_{this->schema_type}_image", $image);
        }
    ...
    protected function inLanguage() {
        ...
        if (!empty($inLanguage)) {
            return apply_filters("markup_{this->schema_type}_inLanguage", $inLanguage);
        }
    ...
    protected function mainEntity( $mainEntity ) {
        ...
        return apply_filters("markup_{this->schema_type}_mainEntity", $mainEntity);
    }
    ...
    protected function hasPart( $hasPart ) {
        ...
        if (!empty($hasPart)) {
            return apply_filters("markup_{this->schema_type}_hasPart", $hasPart);
        }
